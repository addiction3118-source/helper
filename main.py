...
WHITELIST = [
    ...
    # скрипты и утилиты
    "скрипт", "script", "макрос", "vba", "excel", "эксель", "таблиц",
    "дашборд", "dashboard",
    # сайты и веб — любые
    "сайт", "site", "website", "лендинг", "landing",
    "веб-приложен", "web app", "mini app", "мини-приложен", "mvp",
    "интернет-магазин", "магазин", "ecommerce", "e-commerce",
    "wordpress", "вордпресс", "woocommerce", "shopify", "opencart", "опенкарт",
    "верстк", "вёрстк", "html", "css", "фронтенд", "frontend", "бэкенд", "backend",
    # мобильные и десктоп
    "приложен", "мобильн", "android", "андроид", "ios", "айфон", "iphone",
    "flutter", "react native", "desktop", "десктоп",
    # расширения, плагины, игры
    "расширен", "extension", "плагин", "plugin", "хром", "chrome",
    "мини-игр", "браузерная игр", "unity", "godot",
    # языки и стеки — любая разработка
    "python", "питон", "пайтон", "php", "javascript", "js", "node",
    "react", "vue", "django", "flask", "fastapi", "laravel",
    "sql", "база данных", "баз данных", "1с", "1c",
    # код и доработки
    "код", "code", "разработ", "программ", "девелопер", "developer",
    "доработ", "доделать", "исправить", "починить", "багфикс", "баг", "ошибк",
]
# Чёрный список — только то, что вайбкодингом НЕ делается. Широкие слова
# («дизайн», «видео», «3d», «анимац») убраны: они резали нормальные задачи
# («бот для скачивания видео», «анимация на сайте»). Дизайн-заказы и так не
# пройдут белый список, а спорные случаи разрулит ИИ-фильтр.
BLACKLIST = [
    "логотип", "logo", "smm", "видеомонтаж", "монтаж видео",
    "копирайт", "рерайт", "перевод текст", "озвучк", "иллюстрац",
    "наполнение", "seo-текст",
]
...
async def evaluate(session, job: Job) -> bool:
    ...
    # Kwork — отклик платный (коннекты). Режем ТОЛЬКО заказы, чья ссылка ведёт
    # на саму площадку (там бесплатно не откликнуться). Текстовые упоминания
    # «kwork»/«кворк» больше не режем: из-за них терялись нормальные заказы из
    # свободных TG-каналов, где автор просто упомянул площадку («раньше делал
    # на kwork»), хотя откликнуться можно напрямую и бесплатно.
    if "kwork" in job.link.lower():
        return False
